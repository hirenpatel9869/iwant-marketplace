...
@Component({
  selector: 'app-state-management-add',
  templateUrl: './state-management-add.component.html',
  styleUrls: ['./state-management-add.component.scss'],
})
export class StateManagementAddComponent implements OnInit {
...
/*  stateTimeZone = [
  ...
  public createState() {
    this.stateModel.countryId = this.contry.id;
    this.stateModel.countryName = this.contry.name;
    this.spinner = true;
    const stateManagementModel = JSON.parse(JSON.stringify(this.stateModel));
    this.stateManagementService.create(stateManagementModel)
        .subscribe((res) => {
          this.spinner = false;
          setTimeout(() => {
            this.router.navigate(['/state-management/view']);
          }, 500);
          this._snackBar.open('State Added Successfully...', 'close', {
            duration: 2000,
          });
